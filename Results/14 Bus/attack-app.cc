...
static const uint16_t pmuPorts[] = {
    5022, 5023, 5024, 5025, 5026, 5027,
    5028, 5029, 5030, 5031, 5032, 5033,
    5034, 5035, 5036, 4712, 4722
};

bool isPmuTraffic(uint16_t port)
{
    for (uint16_t p : pmuPorts)
...
//			else
         ...
         ( isPmuTraffic(tcpHdr1.GetDestinationPort()) ||
           isPmuTraffic(tcpHdr1.GetSourcePort()) ))
{
	Ipv4Address senderIp;
	uint32_t senderIntIP;
	uint64_t key;
	...
	//printf("DNP3 \n"); // @suppress("Function cannot be resolved")
	packetCopy->CopyData (buffer, dataSize);
	if (isPmuTraffic(tcpHdr1.GetDestinationPort()))
	{
		senderIp = ipV4Hdr.GetSource();
		senderIntIP = senderIp.Get();
		key = (senderIntIP<<16) + tcpHdr1.GetSourcePort();
	}
	else if (isPmuTraffic(tcpHdr1.GetSourcePort()))
	{
		senderIp = ipV4Hdr.GetDestination();
				senderIntIP = senderIp.Get();
				key = (senderIntIP<<16) + tcpHdr1.GetDestinationPort();
